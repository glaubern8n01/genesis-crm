...
serve(async (req) => {
    ...
    if (req.method === "POST") {
        try {
            ...
            const wa_id = message.from; // Número do usuário (ex: 551199999999)
            const name = contactInfo?.profile?.name || wa_id;
            const wamid = message.id;

            // --- TRATAMENTO DE CONTEÚDO (ÁUDIO/IMAGEM/TEXTO) ---
            let textBody = "";
            ...
            if (!contact) {
                ...
                const { data: newContact, error: createError } = await supabase
                    ...
                    .insert({
                        ...
                        name: name,
                        current_stage: "lead",
                        current_step_key: null // Garante start limpo
                    })
                    .select()
                    .single();
            ...
            if (!contact.current_step_key) {
                // >>>> CENÁRIO: INÍCIO DO FUNIL <<<<
                // Tenta achar 'etapa_1' ou 'welcome' ou 'boas_vindas' ou o mais antigo
                ...
                if (steps && steps.length > 0) {
                    // Prioridade: começa com 'etapa_1', depois 'welcome', depois o primeiro da lista
                ...
                if (currentStep) {
                    if (currentStep.next_step) {
                        // Buscar o próximo passo explicitamente
                        ...
                        if (next) {
                            ...
                            console.warn(`Próximo passo '${currentStep.next_step}' não encontrado no banco.`);
                        }
                    } else {
                        console.log("Fim do funil (sem next_step).");
                    }
                } else {
                    // Passo atual deletado ou inválido? Reinicia?
                    // Melhor não responder nada do que responder boas vindas de novo sem contexto.
                    console.warn(`Passo atual '${contact.current_step_key}' não existe mais.`);
                }
            }

            // 6. EXECUTAR PASSO
            if (targetStep) {
                console.log(`Executando passo: ${targetStep.step_key} (Anterior: ${contact.current_step_key})`);
                ...
                if (updateError) {
                    ...
                    console.log(`Contato atualizado para step: ${targetStep.step_key}`);
                }
...
async function sendFunnelAudio(to: string, storagePath: string) {
    ...
    try {
        ...
        let msgType = "audio";
        let msgContent: any = { id: mediaId };

        if (mimeType.includes("video")) {
            msgType = "video";
            msgContent = { id: mediaId };
        }

        if (mediaId) {
            await sendMessage(to, { type: msgType, [msgType]: msgContent });
        } else {
    ...
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "user", content: [
                        { type: "text", text: "Descreva. Se for erro de pagamento/pix/boleto, diga 'ERRO PAGAMENTO'." },
...
async function uploadMediaToWhatsApp(bytes: ArrayBuffer, mimeType: string): Promise<string | null> {
    ...
    let ext = "bin";
    // WhatsApp exige extensão correta no nome do arquivo para aceitar o upload como audio/video válido
